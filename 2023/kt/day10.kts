tiles = System.`in`.bufferedReader().readLines().map { it.toByteArray() }
...
for (r in 0 ..< (tiles.size * 2)) {
  for (c in 0 ..< (tiles[0].size * 2)) {
    if (loop2.contains(Pair(c + 1, r)) &&
            loop2.contains(Pair(c - 1, r)) &&
            "L-FS".contains(tile((c - 1) / 2, r / 2)) &&
            "J-7S".contains(tile((c + 1) / 2, r / 2))
    ) {
      loop2.add(Pair(c, r))
    }
    if (loop2.contains(Pair(c, r - 1)) &&
            loop2.contains(Pair(c, r + 1)) &&
            "7|FS".contains(tile(c / 2, (r - 1) / 2)) &&
            "J|LS".contains(tile(c / 2, (r + 1) / 2))
    ) {
      loop2.add(Pair(c, r))
    }
  }
...
fun inner(loop: Set<Pair<Int, Int>>, pos: Pair<Int, Int>): Boolean {
  ...
  while (!queue.isEmpty()) {
    ...
    visited.add(p)
    if (p.first < 0 || p.second < 0 || p.first >= tiles[0].size * 2 || p.second >= tiles.size * 2)
        return false
