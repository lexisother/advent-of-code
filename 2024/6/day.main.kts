...
val (initialWalk, timeToWalk) = measureTimedValue {
    input.walk(initialPosition).mapTo(mutableSetOf()) { it.first }
}

println("Part 1: ${initialWalk.size} (${timeToWalk.inWholeMilliseconds}ms)")

// async because we're using a flow to compute our amount
runBlocking {
    launch {
        // TODO: In the future, figure out why this is so awfully slow
        val (amountOfLoopPositions, timeToCalculate) = measureTimedValue {
            @OptIn(ExperimentalCoroutinesApi::class)
            initialWalk.asFlow().drop(1).flatMapMerge { (y, x) ->
        ...
        println("Part 2: $amountOfLoopPositions (${timeToCalculate.inWholeMilliseconds}ms)")
    }
}
